...
describe('Assignment 3 - async', function() {
	...
	it('JS file should exist, and should have named exports', function() {
		const { getTatooineResidents, promiseMeAString } = require('./async-functions');
		assert.strictEqual(
	...
	it('getTatooineResidents should make a request to https://swapi.co/api/planets/1/', function(done) {
		...
		const api = nock('https://swapi.co/api').get('/planets/1/').reply(200, planets1);
		...
		try {
			...
			assert.strictEqual(api.isDone(), true, 'No request has been made to https://swapi.co/api/planets/1/');
	...
	it('getTatooineResidents shoud return a promise', function(done) {
		// Intercept request and respond with a fake response
		nock('https://swapi.co/api').get('/planets/1/').reply(200, planets1);
	...
	it('getTatooineResidents should return a promise which resolves with an array of urls for the residents of Tatooine', function(
		done
	) {
		// Intercept request and respond with a fake response
		const api = nock('https://swapi.co/api').get('/planets/1/').reply(200, planets1);
	...
	it('when promiseMeAString a called with a string as an argument it should return a Promise which resolves with a string of "You kept the Promise!"', function(
		done
	) {
		// Import function
		const { promiseMeAString } = require('./async-functions');
	...
	it('when promiseMeAString a called with a null as an argument it should return a Promise which rejects with a string of "You have failed me!"', function(
		done
	) {
		// import function
		const { promiseMeAString } = require('./async-functions');
